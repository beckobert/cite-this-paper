...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Classify terminal end matter and exclude it from evidence retrieval."
        )
    )
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help=("Print detailed information about detected terminal-section cutoffs."),
    )

    args = parser.parse_args()

    passages = load_jsonl(args.passages_jsonl.resolve())

    sentences = load_sentences(args.sentences_jsonl.resolve())

    documents = load_documents(args.documents_jsonl.resolve())
    ...
    passages_by_document = defaultdict(list)
    ...
    for document_id, document_passages in passages_by_document.items():
    ...
    for report in reports:
        ...
        if cutoff is None:
            print("    cutoff: none detected")

        else:
            print(f"    cutoff: page {cutoff['page_number']}")

            print(f"    reason: {cutoff['reason']}")

            if cutoff["reason"] == "terminal_heading":
                print(f"    section: {cutoff['section_type']}")

            else:
                print(f"    reference diagnostics: {cutoff['diagnostics']}")

        print(
            f"    filtered: "
            f"{report['filtered_passages']} / "
            f"{report['passages']} passages"
        )

        total_filtered += report["filtered_passages"]

        if args.debug and cutoff is not None:
            print()
            print("    DEBUG")

            if cutoff["reason"] == "terminal_heading":
                print(f"        matched text       : {cutoff['matched_text']!r}")

                print(f"        match type         : {cutoff['match_type']}")

                print(f"        heading char       : {cutoff['heading_position']}")

                print(
                    f"        relative position  : "
                    f"{cutoff['heading_relative_position']:.1%}"
                )

                print(f"        filtering begins   : {cutoff['filtering_begins']}")
            ...
            elif cutoff["reason"] == "horizontal_rule_with_reference_like_text":
                print(f"        separator y        : {cutoff['y']:.1f}")

                print(f"        diagnostics        : {cutoff['diagnostics']}")

    print()
    print("=" * 80)
    print(f"Documents: {len(reports)}")
    print(f"Passages filtered: {total_filtered}")
    print()
