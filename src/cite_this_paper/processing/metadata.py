...
# PyMuPDF exposes the historic ``fitz`` API, used throughout the prototype.
# Keeping that local alias lets this package port retain identical extraction
# behavior without depending on the deprecated import name.
...
def extract_identifiers(
    ...
    for match in ISSN_RE.finditer(full_text):
        identifier = match.group("id").upper()

        if match.group("label").casefold().startswith("e"):
            eissn.add(identifier)
    ...
    eissn.update(value.upper() for value in xmp["eissn"])
...
def extract_document_metadata(
    ...
    second_page_lines = [line for line in front_page_lines if line.page_index == 1]
    ...
    # The prototype deliberately treats PDF page 1 as the article front page.
    # Some publisher files add a cover page, however, so retain its selection
    # behavior and only fall back to page 2 when page 1 cannot resolve a title.
    author_page_lines = first_page_lines

    if title["value"] is None and second_page_lines:
        second_page_title = select_title(
            document=document,
            xmp=xmp,
            first_page_lines=(second_page_lines),
            known_dois=(known_dois),
        )

        if second_page_title["value"] is not None:
            title = second_page_title
            author_page_lines = second_page_lines
    ...
    authors = select_authors(
        document=document,
        xmp=xmp,
        first_page_lines=(author_page_lines),
        title_result=title,
    )
...
def review_payload(metadata: dict) -> dict:
    """Return prototype diagnostics suitable for persistent metadata review.

    Scores, sources, confidence, page references, and alternatives are retained;
    raw layout geometry is deliberately removed from the stored representation.
    """
    def sanitize(value):
        ...
        for key, item in value.items():
            if key in {"bbox", "font_size", "width_fraction", "title_bbox"}:
                continue
            if key == "page_index":
                result["page_number"] = int(item) + 1
            else:
